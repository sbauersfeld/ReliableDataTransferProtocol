...
void Wrap_inc(int &value, int amt) {
	value = ((value + amt) % (MAX_SEQ + 1)); //keeps sequence numbers in range [0,30720]
}
...
void SendtoC(int socknum, segment * buf, size_t len, int flags, const struct sockaddr *dest_addr, socklen_t dest_len, bool R, int cwnd) {
	int ret;
	...
	if (R && buf->connection == SYN) {
		fprintf(stdout, "Sending packet %d %d Retransmission SYN\n", buf->seq, cwnd*PKT_SIZE);
	}
	else if (R && buf->connection == FIN) {
		fprintf(stdout, "Sending packet %d %d Retransmission FIN\n", buf->seq, cwnd*PKT_SIZE);
	}
	else if (R) {
		fprintf(stdout, "Sending packet %d %d Retransmission\n", buf->seq, cwnd*PKT_SIZE);
	}
	else if (buf->connection == SYN) {
		fprintf(stdout, "Sending packet %d %d SYN\n", buf->seq, cwnd*PKT_SIZE);
	}
	else if (buf->connection == FIN) {
		fprintf(stdout, "Sending packet %d %d FIN\n", buf->seq, cwnd*PKT_SIZE);
	}
	else {
		fprintf(stdout, "Sending packet %d %d\n", buf->seq, cwnd*PKT_SIZE);
	}
}
...
void SendtoS(int socknum, segment * buf, size_t len, int flags, const struct sockaddr *dest_addr, socklen_t dest_len, bool R) {
	...
	if (R && buf->connection == SYN) {
		fprintf(stdout, "Sending packet %d Retransmission SYN\n", buf->seq);
	}
	else if (R && buf->connection == FIN) {
		...
		fprintf(stdout, "Sending packet %d SYN\n", buf->seq);
	}
	else if (buf->connection == FIN) {
