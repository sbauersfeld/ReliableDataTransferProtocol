...
void Connect(int socknum, struct sockaddr_in &client) {
    ...
    SendtoC(socknum, &buf2, buf2.length, 0, (struct sockaddr *) &client, sizeof(client), 0, cwnd, ssthresh);
    ...
    while (1) {
        ...
        if (duration >= RTO) { //resend packet after a time out
            SendtoC(socknum, &buf2, buf2.length, 0, (struct sockaddr *) &client, sizeof(client), 1, cwnd, ssthresh);
        ...
        if (rc) { //read from socket if data is available
			...
			if (valid && buf.ack == serv_seq) {
                printReceive(expected_ack + buf.length - HEADER_SIZE);
				cwnd++;
				break;
			} else {
                printReceive(expected_ack);
...
void CloseConnection(int socknum, struct sockaddr_in &client) {
    ...
    SendtoC(socknum, &pkt, pkt.length, 0, (struct sockaddr *) &client, sizeof(client), 0, cwnd, ssthresh);
    ...
    while (1) {
        ...
        if (duration >= RTO) { //resend packet after a time out
            ...
            SendtoC(socknum, &pkt, pkt.length, 0, (struct sockaddr *) &client, sizeof(client), 1, cwnd, ssthresh);
            start = clock(); //reset clock
        }
...
void sendFile(char *file, int socknum, struct sockaddr_in &client) {
    ...
    while (1){
        ...
        if (duration >= RTO) {
			...
			if (cwnd / 2 < 2)
				ssthresh = 2;
			else ssthresh = cwnd / 2;

			cwnd = 1;
			congestion_counter = 0;
			state = SS;
            SendtoC(socknum, &packets.front(), (&packets.front())->length, 0, (struct sockaddr *) &client, sizeof(client), 1, cwnd, ssthresh);
        }
        ...
        if (rc) { 
			...
			int64_t v1 = 0xFFFFFFFF; //max sequence number
			int64_t v2 = packets.front().seq;
            int64_t diff = v1 - v2;
            int64_t end_win = win - diff; //used to calculate if acks could wrap around max seq value
		if (packets.empty())
			end_win = -1;
            if (valid && !packets.empty() && ((rcvPkt.ack > packets.front().seq && rcvPkt.ack <= packets.front().seq + win) || ((end_win > 0) && (rcvPkt.ack < end_win)))){ //expected_ack <?
				...
				while ((packets.front().seq < rcvPkt.ack && rcvPkt.ack <= packets.front().seq + win) || ((end_win > 0) && (rcvPkt.ack < end_win))) { //expected_ack
					...
					win2 -= packets.front().length;
					packets.pop();
					if (count < cwnd && win < MAX_CWND)
						count++;
                    shouldRestartClock = true;
					start = clock();

					v2 = packets.front().seq;
					diff = v1 - v2;
            ...
            else if (valid && !packets.empty() && rcvPkt.ack == packets.front().seq) { //Fast Recovery
				printReceive(expected_ack);
				if (state == FR) { //additional duplicates for fast recovery
					if (cwnd*PKT_SIZE < MAX_CWND) {
					cwnd++;
					if (win < MAX_CWND)
						count++;
				...
				if (state != FR) //count duplicate
					triple_dup++;
                if (triple_dup == 3) { //Fast retransmit
					state = FR;
					if (cwnd / 2 < 2)
						ssthresh = 2;
					else ssthresh = cwnd / 2;
					congestion_counter = 0;
					cwnd = ssthresh + 3;
                    SendtoC(socknum, &packets.front(), pkt.length, 0, (struct sockaddr *) &client, sizeof(client), 1, cwnd, ssthresh);
                    triple_dup = 0;
                    start = clock();
                }
            }
