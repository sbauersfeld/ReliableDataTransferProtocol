...
unsigned long amtReceived = 0;  // Amount of bytes received from server
...
void receiveFile(int fd, int socknum, struct sockaddr_in &server) {
	...
	int count, max;
	max = 4*RTO; //just so we dont loop forever if final ack is lost
	count = 0;
	...
	while (1) {
		...
		if (start_counting && duration >= RTO) {
			count += RTO;
			if (count >= max && amtReceived >= (unsigned) fileSize) {  // If FIN doesn't arrive for a long time but we received all data, exit
				break;
			}
			start = clock();
		}

		if (amtReceived >= (unsigned) fileSize && !start_counting) {
			start = clock();
			start_counting = true;
		}
		...
		if (rc) { //read from socket if data is available
			...
			v += 1;
			end_win = end_win % v; //used to calculate if acks could wrap around max seq value
			if (win > 0xFFFFFFFF)
				cout << "test" << endl;
			...
			if (valid && rcvPkt.seq == acknum) { //if the next packet we get is the base sequence number of client window
				if (rcvPkt.connection == FIN) {//no more data is coming
					Wrap_inc(acknum, 1);
					printReceive(acknum);
					receivedFIN = true;
					break;
			...
			else if (valid && ((rcvPkt.seq > acknum  && rcvPkt.seq <= acknum + 2*MAX_CWND) || ((win > 0xFFFFFFFF) && (rcvPkt.seq <= end_win)))) { //buffer out of order packets
				list<segment>::iterator seg_pointer, end_pointer;
				seg_pointer = rcvPackets.begin();
				end_pointer = rcvPackets.end();
				bool dup = false;
...
int main(int argc, char **argv){
	...
	memcpy((char *) &server.sin_addr.s_addr, (char *)serverHost->h_addr, serverHost->h_length);
